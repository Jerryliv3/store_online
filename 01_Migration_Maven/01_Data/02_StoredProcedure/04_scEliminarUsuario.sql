...
GO
  ...
  /*----------------------------------------------------------------------------  
// Stored Procedure: [scEliminarUsuario].  
//         Objetivo: Elimina Entidad Usuario
//            Fecha: 09 Mayo 2023.  
...
//-----------------------------------------------------------------------------*/  
CREATE PROCEDURE [dbo].[scEliminarUsuario] (  
...
BEGIN	
 ...
 BEGIN TRY  
   ...
   SELECT	 Tbl.Col.value('usuarioId[1]', 'int') usuarioId,  
	...
	DELETE FROM USUARIO WHERE UsuarioId = ( SELECT usuarioId FROM #TempUsuario )
